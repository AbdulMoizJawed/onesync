...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${BASE_URL}/tracks/${isrc}/spotify/streams`, {
      headers: {
    ...
    console.error('Spotify streams error:', error)
    return NextResponse.json({ error: 'Failed to fetch Spotify streams' }, { status: 500 })
  }
}
