...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${BASE_URL}/tracks/${isrc}/apple-music/playlists/current`, {
      headers: {
    ...
    return NextResponse.json({ error: 'Failed to fetch Apple Music playlists' }, { status: 500 })
  }
}
