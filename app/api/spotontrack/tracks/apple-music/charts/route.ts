...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${BASE_URL}/tracks/${isrc}/apple-music/charts/current`, {
      headers: {
    ...
    console.error('Apple Music charts error:', error)
    return NextResponse.json({ error: 'Failed to fetch Apple Music charts' }, { status: 500 })
  }
}
