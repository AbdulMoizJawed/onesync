...
export default function AdminDashboard() {
  ...
  const [playlistCampaigns, setPlaylistCampaigns] = useState<PlaylistCampaign[]>([])
    ...
    try {
      if (action === 'suspend') {
        ...
        await sendUserNotification(
          userId,
          'Account Suspended',
          `Your account has been ${suspensionType === 'permanent' ? 'permanently' : 'temporarily'} suspended. Reason: ${reason}${suspensionType === 'temporary' ? ' (30 days)' : ''}`,
          'warning'
        )
  ...
  const fetchStats = async () => {
    try {
      ...
      const [
        ...
        supabase.from('releases').select('id', { count: 'exact' }).in('status', ['pending', 'draft', 'processing']),
        supabase.from('support_requests').select('id', { count: 'exact' }).eq('status', 'open'),
        supabase.from('publishing_requests').select('id', { count: 'exact' }).eq('status', 'pending'),
        supabase.from('playlist_campaigns').select('id', { count: 'exact' }).in('status', ['pending', 'in_progress']),
        supabase.from('beats').select('id', { count: 'exact' }).eq('approval_status', 'pending'),
        supabase.from('mastering_jobs').select('id', { count: 'exact' }).in('status', ['pending', 'processing']),
        supabase.from('forum_content_flags').select('id', { count: 'exact' }).eq('status', 'pending')
      ])
  ...
  const sendUserNotification = async (userId: string, title: string, message: string, type: 'release' | 'payout' | 'system' | 'warning' = 'system') => {
    try {
      const { error } = await supabase
        .from('notifications')
        .insert({
  ...
  const handleSupportAction = async (ticketId: string, status: 'in_progress' | 'resolved' | 'closed') => {
    try {
      ...
      toast.success(`Support ticket ${status.replace('_', ' ')} and user notified`)
      fetchSupportTickets()
    } catch (error) {
  ...
  const handlePublishingAction = async (requestId: string, action: 'approve' | 'reject', adminNotes?: string) => {
    try {
      const response = await fetch('/api/admin/publishing-requests', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: requestId, action, admin_notes: adminNotes })
      })
        ...
        if (request) {
          await sendUserNotification(
            ...
            action === 'approve'
              ...
              : `Your publishing administration request has been rejected. ${adminNotes ? `Reason: ${adminNotes}` : 'Please contact support for more information.'}`,
            action === 'approve' ? 'system' : 'warning'
          )
        }

        toast.success(`Publishing request ${action}d successfully and user notified`)
        fetchPublishingRequests()
                ...
                <div className="space-y-4">
                  {releases.length === 0 ? (
                    ...
                    releases.map((release) => (
                      <div key={release.id} className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
                        <div className="flex items-center justify-between p-4">
                          <div className="flex items-center space-x-4">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-gray-400 text-sm">{release.artist || release.artist_name}</p>
                              <p className="text-gray-500 text-xs">{release.genre}</p>
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            >
                              ...
                              {selectedRelease?.id === release.id ? 'Hide' : 'Details'}
                            </Button>
                            <Button
                              size="sm"
                              variant="outline"
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  a.href = url
                                  a.download = `${release.title.replace(/[^a-z0-9]/gi, '_')}_complete.zip`
                        ...
                        {selectedRelease?.id === release.id && (
                          <div className="border-t border-gray-700 p-4 bg-gray-850">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                              <div>
                                <h4 className="text-white font-medium mb-2">Release Information</h4>
                                <div className="space-y-1 text-sm">
                                  <p><span className="text-gray-400">Release Date:</span> <span className="text-white">{release.release_date ? new Date(release.release_date).toLocaleDateString() : 'Not set'}</span></p>
                                  <p><span className="text-gray-400">Genre:</span> <span className="text-white">{release.genre || 'Not specified'}</span></p>
                                  <p><span className="text-gray-400">Type:</span> <span className="text-white capitalize">{release.type || release.metadata?.releaseType || 'Single'}</span></p>
                                  {release.track_count && <p><span className="text-gray-400">Tracks:</span> <span className="text-white">{release.track_count}</span></p>}
                                  <p><span className="text-gray-400">User ID:</span> <span className="text-white font-mono text-xs">{release.user_id}</span></p>
                              ...
                              <div>
                                ...
                                <div className="space-y-1 text-sm">
                                  <p><span className="text-gray-400">Streams:</span> <span className="text-white">{(release.streams || 0).toLocaleString()}</span></p>
                                  <p><span className="text-gray-400">Revenue:</span> <span className="text-white">${(release.revenue || 0).toFixed(2)}</span></p>
                                  <p><span className="text-gray-400">Platforms:</span> <span className="text-white">{release.platforms?.length || 0} platforms</span></p>
                              ...
                              <div>
                                ...
                                <div className="space-y-1 text-sm">
                                  ...
                                  <p><span className="text-gray-400">Created:</span> <span className="text-white">{new Date(release.created_at).toLocaleString()}</span></p>
                                ...
                                <div className="md:col-span-2 lg:col-span-3">
                                  ...
                                  <div className="bg-gray-900 rounded p-3">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                      {Object.entries(release.metadata).map(([key, value]) => (
              ...
              <CardContent>
                <div className="space-y-4">
                  {users
                    ...
                    .map((user) => (
                      <div key={user.id} className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
                        <div className="flex items-center justify-between p-4">
                          <div className="flex items-center space-x-4">
                          ...
                          <div className="border-t border-gray-700 p-4 bg-gray-850">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                              <div>
                                ...
                                <div className="space-y-1 text-sm">
                                  <p><span className="text-gray-400">Full Name:</span> <span className="text-white">{user.full_name || 'Not provided'}</span></p>
                                  <p><span className="text-gray-400">Email:</span> <span className="text-white">{user.email}</span></p>
                                  <p><span className="text-gray-400">Role:</span> <span className="text-white">{user.role || 'User'}</span></p>
                                  <p><span className="text-gray-400">Status:</span> <span className="text-white">{user.status || 'Active'}</span></p>
                              ...
                              <div>
                                ...
                                <div className="space-y-1 text-sm">
                                  <p><span className="text-gray-400">Joined:</span> <span className="text-white">{new Date(user.created_at).toLocaleDateString()}</span></p>
                                  <p><span className="text-gray-400">Last Active:</span> <span className="text-white">{user.last_sign_in_at ? new Date(user.last_sign_in_at).toLocaleDateString() : 'Never'}</span></p>
                                  <p><span className="text-gray-400">Total Releases:</span> <span className="text-white">{user.release_count || 0}</span></p>
                              ...
                              <div>
                                ...
                                <div className="space-y-2">
                                  ...
                                  <Button
                                    ...
                                    variant="outline"
                                    className="w-full border-gray-600 text-gray-300 hover:bg-gray-700"
                                    onClick={() => handleViewUserReleases(user.id)}
                                  >
                                  ...
                                  <Button
                                    ...
                                    variant="outline"
                                    className="w-full border-gray-600 text-gray-300 hover:bg-gray-700"
                                    onClick={() => handleViewUserPayouts(user.id)}
                                  >
                ...
                <div className="space-y-4">
                  {payoutRequests.length === 0 ? (
                    ...
                    payoutRequests.map((payout) => (
                      <div key={payout.id} className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
                        <div className="flex items-center justify-between p-4">
                          <div className="flex items-center space-x-4">
                            <div className="w-12 h-12 bg-gray-700 rounded-full flex items-center justify-center">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-gray-400 text-sm">{payout.method_type || payout.payment_method}</p>
                              <p className="text-gray-500 text-xs">User: {payout.user_email || payout.user_id}</p>
                              <div className="flex items-center space-x-2 mt-1">
                                <Badge
                                  variant={
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            >
                              <Eye className="w-4 h-4 mr-1" />
                              {selectedPayout?.id === payout.id ? 'Hide' : 'Details'}
                            </Button>
                          </div>
                        </div>
                          ...
                          <div className="border-t border-gray-700 p-4 bg-gray-850">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                              <div>
                                ...
                                <div className="space-y-1 text-sm">
                                  <p><span className="text-gray-400">Amount:</span> <span className="text-white">${payout.amount.toFixed(2)}</span></p>
                                  <p><span className="text-gray-400">Method:</span> <span className="text-white">{payout.method_type || payout.payment_method}</span></p>
                                  <p><span className="text-gray-400">Status:</span> <span className="text-white">{payout.status}</span></p>
                                  <p><span className="text-gray-400">User ID:</span> <span className="text-white font-mono text-xs">{payout.user_id}</span></p>
                              ...
                              <div>
                                ...
                                <div className="space-y-1 text-sm">
                                  {payout.method_type === 'paypal' && payout.paypal_email && (
                                    <p><span className="text-gray-400">PayPal Email:</span> <span className="text-white">{payout.paypal_email.replace(/(.{2}).*(@.*)/, '$1***$2')}</span></p>
                                  )}
                                  {payout.method_type === 'bank_transfer' && (
                                    <>
                                      {payout.bank_name && <p><span className="text-gray-400">Bank:</span> <span className="text-white">{payout.bank_name}</span></p>}
                                      {payout.account_number && <p><span className="text-gray-400">Account:</span> <span className="text-white">***{payout.account_number.slice(-4)}</span></p>}
                                    </>
                                  )}
                                  <p><span className="text-gray-400">Created:</span> <span className="text-white">{new Date(payout.created_at).toLocaleString()}</span></p>
                                  {payout.processed_at && (
                                    <p><span className="text-gray-400">Processed:</span> <span className="text-white">{new Date(payout.processed_at).toLocaleString()}</span></p>
                                  )}
                                </div>
                              </div>
                              ...
                              <div>
                                ...
                                <div className="space-y-2">
                                  {payout.status === 'pending' && (
                                    <>
                                      ...
                                      <Button
                                        ...
                                        variant="destructive"
                                        className="w-full"
                                        onClick={() => handlePayoutAction(payout.id, 'rejected', 'Manual review required')}
                                      >
                                  ...
                                  <Button
                                    ...
                                    variant="outline"
                                    className="w-full border-gray-600 text-gray-300 hover:bg-gray-700"
                                    onClick={() => handleViewUserPayouts(payout.user_id)}
                                  >
                              ...
                              {payout.admin_notes && (
                                <div className="md:col-span-2 lg:col-span-3">
                                  ...
                                  <div className="bg-gray-900 rounded p-3">
                                    <p className="text-gray-300 text-sm">{payout.admin_notes}</p>
                                  </div>
                                </div>
                              )}
                            </div>
          ...
          <TabsContent value="edits" className="space-y-6">
            <Card className="bg-gray-900 border-gray-800">
              ...
              <CardContent>
                <div className="space-y-4">
                  {releaseEdits
                    .filter(edit => statusFilter === 'all' || edit.status === statusFilter)
                    .length === 0 ? (
                    <div className="text-center py-8">
                    ...
                    releaseEdits
                      .filter(edit => statusFilter === 'all' || edit.status === statusFilter)
                      .map((edit) => (
                        <div key={edit.id} className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
                          <div className="flex items-center justify-between p-4">
                            <div className="flex items-center space-x-4">
                              <div className="w-12 h-12 bg-gray-700 rounded-full flex items-center justify-center">
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-gray-400 text-sm">
                                  by {edit.releases?.artist_name || 'Unknown Artist'}
                                </p>
                                <p className="text-gray-500 text-xs">
                                  Edited by: {edit.profiles?.full_name || edit.profiles?.email || 'Unknown User'}
                                </p>
                                <div className="flex items-center space-x-2 mt-1">
                                  <Badge
                                    variant={
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <Button
                                ...
                                variant="outline"
                                className="border-gray-600 text-gray-300 hover:bg-gray-700"
                                onClick={() => setSelectedRelease(selectedRelease?.id === edit.id ? null : edit)}
                              >
                                <Eye className="w-4 h-4 mr-1" />
                                {selectedRelease?.id === edit.id ? 'Hide' : 'Changes'}
                              </Button>
                            </div>
                          </div>
                          ...
                          {selectedRelease?.id === edit.id && (
                            <div className="border-t border-gray-700 p-4 bg-gray-850">
                              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div>
                                  <h4 className="text-white font-medium mb-3">Original Data</h4>
                                  <div className="bg-gray-900 rounded p-3 space-y-2">
                                    {edit.original_data && Object.entries(edit.original_data).map(([key, value]) => (
                                      <div key={key} className="text-sm">
                                        <span className="text-gray-400 capitalize">
                                          {key.replace(/_/g, ' ')}:
                                        </span>
                                ...
                                <div>
                                  ...
                                  <div className="bg-gray-900 rounded p-3 space-y-2">
                                    {edit.proposed_changes && Object.entries(edit.proposed_changes).map(([key, value]) => (
                                      <div key={key} className="text-sm">
                                        <span className="text-gray-400 capitalize">
                                          {key.replace(/_/g, ' ')}:
                                        </span>
                                  ...
                                  {edit.rejection_reason && (
                                    <div className="mt-3">
                                      ...
                                      <div className="bg-red-900/20 border border-red-800 rounded p-3">
                                        <p className="text-red-300 text-sm">{edit.rejection_reason}</p>
                                      </div>
                                    </div>
                                  )}
                                  ...
                                  <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                    ...
                                    {edit.applied_at && (
                                      <div>
                                        ...
                                        <span className="text-white ml-2">
                                          {new Date(edit.applied_at).toLocaleString()}
                                        </span>
                                      </div>
                                    )}
                                  </div>
                ...
                <div className="space-y-4">
                  {supportTickets
                    ...
                    supportTickets
                      .filter(ticket => statusFilter === 'all' || ticket.status === statusFilter)
                      .map((ticket) => (
                        <div key={ticket.id} className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
                          <div className="flex items-center justify-between p-4">
                            <div className="flex items-center space-x-4">
                              <div className="w-12 h-12 bg-gray-700 rounded-full flex items-center justify-center">
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-gray-400 text-sm">
                                  by {ticket.profiles?.full_name || ticket.profiles?.email || 'Unknown User'}
                                </p>
                            ...
                            <div className="flex items-center space-x-2">
                              {ticket.status === 'open' && (
                                <Button
                                  size="sm"
                                  onClick={() => handleSupportAction(ticket.id, 'in_progress')}
                              ...
                              {ticket.status === 'in_progress' && (
                                <Button
                                  size="sm"
                                  onClick={() => handleSupportAction(ticket.id, 'resolved')}
                                  className="bg-green-600 hover:bg-green-700"
                                >
                                  <CheckCircle className="w-4 h-4 mr-1" />
                                  Resolve
                              ...
                              <Button
                                ...
                                variant="outline"
                                className="border-gray-600 text-gray-300 hover:bg-gray-700"
                                onClick={() => setSelectedRelease(selectedRelease?.id === ticket.id ? null : ticket)}
                              >
                                <Eye className="w-4 h-4 mr-1" />
                                {selectedRelease?.id === ticket.id ? 'Hide' : 'Details'}
                              </Button>
                            </div>
                          </div>
                          ...
                          {selectedRelease?.id === ticket.id && (
                            <div className="border-t border-gray-700 p-4 bg-gray-850">
                              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div>
                                  <h4 className="text-white font-medium mb-3">Ticket Details</h4>
                                  <div className="space-y-2 text-sm">
                                    <p><span className="text-gray-400">Subject:</span> <span className="text-white">{ticket.subject}</span></p>
                                    <p><span className="text-gray-400">Category:</span> <span className="text-white">{ticket.category}</span></p>
                                    <p><span className="text-gray-400">Priority:</span> <span className="text-white">{ticket.priority}</span></p>
                                    <p><span className="text-gray-400">Status:</span> <span className="text-white">{ticket.status.replace('_', ' ')}</span></p>
                                ...
                                <div>
                                  ...
                                  <div className="space-y-2 text-sm">
                                    <p><span className="text-gray-400">Created:</span> <span className="text-white">{new Date(ticket.created_at).toLocaleString()}</span></p>
                                    <p><span className="text-gray-400">Updated:</span> <span className="text-white">{new Date(ticket.updated_at).toLocaleString()}</span></p>
                                    {ticket.resolved_at && (
                                      <p><span className="text-gray-400">Resolved:</span> <span className="text-white">{new Date(ticket.resolved_at).toLocaleString()}</span></p>
                                    )}
                                    {ticket.assigned_to && (
                                      <p><span className="text-gray-400">Assigned to:</span> <span className="text-white">{ticket.assigned_to}</span></p>
                                    )}
                                  </div>
                                </div>
                                ...
                                <div className="lg:col-span-2">
                                  ...
                                  {ticket.internal_notes && (
                                    <div className="mt-3">
                                      ...
                                      <div className="bg-yellow-900/20 border border-yellow-800 rounded p-3">
                                        <p className="text-yellow-300 text-sm">{ticket.internal_notes}</p>
                                      </div>
                                    </div>
                                  )}
                                  ...
                                  {ticket.resolution_notes && (
                                    <div className="mt-3">
                                      ...
                                      <div className="bg-green-900/20 border border-green-800 rounded p-3">
                                        <p className="text-green-300 text-sm">{ticket.resolution_notes}</p>
                                      </div>
                                    </div>
                                  )}
                                </div>
                    ...
                    <div key={campaign.id} className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          ...
                          <div className="text-gray-300 text-sm space-y-1">
                            <div>Artist: {campaign.releases?.artist_name || 'Unknown'}</div>
                            ...
                            {campaign.started_at && <div>Started: {new Date(campaign.started_at).toLocaleDateString()}</div>}
                            {campaign.completed_at && <div>Completed: {new Date(campaign.completed_at).toLocaleDateString()}</div>}
                          </div>
                        </div>
                        ...
                        {campaign.status === 'in_progress' && (
                          <Button
                            size="sm"
                            onClick={async () => {
                              const placements = prompt('Enter number of playlist placements:')
                              ...
                              await fetch('/api/admin/playlist-campaigns', {
                                ...
                                body: JSON.stringify({
                                  ...
                                  data: {
                                    ...
                                    streams_gained: streams ? parseInt(streams) : 0
                                  }
                                })
                              })
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          ...
                          <div className="text-gray-300 text-sm space-y-1">
                            ...
                            {job.api_cost && <div>API Cost: ${job.api_cost.toFixed(2)}</div>}
                          </div>
                          {job.admin_notes && (
                            <div className="mt-3 p-3 bg-gray-900 rounded">
                              <p className="text-gray-400 text-xs">Admin Notes: {job.admin_notes}</p>
                            </div>
                          )}
                        </div>
                        <div className="flex space-x-2 ml-4">
          ...
          <TabsContent value="forum-mod" className="space-y-6">
            <Card className="bg-gray-900 border-gray-800">
              ...
              <CardContent>
                <div className="space-y-4">
                  {forumFlags.map((flag) => (
                    <div key={flag.id} className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-2">
                            <Badge className="bg-red-600">{flag.content_type}</Badge>
                            <Badge variant="outline" className="text-orange-400 border-orange-400">
                              {flag.flag_reason}
                            </Badge>
                            <Badge className={
                          ...
                          <div className="text-gray-300 text-sm space-y-1">
                            ...
                            <div>Reported: {new Date(flag.created_at).toLocaleDateString()}</div>
                            {flag.description && <div className="mt-2 italic">"{flag.description}"</div>}
                          </div>
                        </div>
                        {flag.status === 'pending' && (
                          <div className="flex space-x-2 ml-4">
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                await fetch('/api/admin/forum-moderation', {
                                  method: 'PATCH',
                                  headers: { 'Content-Type': 'application/json' },
                                  body: JSON.stringify({ id: flag.id, action: 'action', admin_notes: notes, admin_action: 'removed' })
                                })
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                await fetch('/api/admin/forum-moderation', {
                                  method: 'PATCH',
                                  headers: { 'Content-Type': 'application/json' },
                                  body: JSON.stringify({ id: flag.id, action: 'dismiss' })
                                })
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-white">${(financialStats?.stats?.totalRevenue || 0).toFixed(2)}</p>
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-white">${(financialStats?.stats?.totalPlatformFees || 0).toFixed(2)}</p>
                    </div>
              ...
              <Card className="bg-gray-900 border-gray-800">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-gray-400 text-sm">Transactions (30d)</p>
                      <p className="text-3xl font-bold text-white">{financialStats?.stats?.totalTransactions || 0}</p>
                    </div>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                  <div className="bg-gray-800 p-4 rounded-lg">
                    ...
                    <p className="text-white text-xl font-bold">${(financialStats?.stats?.byServiceCategory?.beat_purchase || 0).toFixed(0)}</p>
                  </div>
                  <div className="bg-gray-800 p-4 rounded-lg">
                    <p className="text-gray-400 text-xs mb-1">Playlist Campaigns</p>
                    <p className="text-white text-xl font-bold">${(financialStats?.stats?.byServiceCategory?.playlist_campaign || 0).toFixed(0)}</p>
                  </div>
                  <div className="bg-gray-800 p-4 rounded-lg">
                    <p className="text-gray-400 text-xs mb-1">Mastering</p>
                    <p className="text-white text-xl font-bold">${(financialStats?.stats?.byServiceCategory?.mastering || 0).toFixed(0)}</p>
                  </div>
                  <div className="bg-gray-800 p-4 rounded-lg">
                    <p className="text-gray-400 text-xs mb-1">Promo Services</p>
                    <p className="text-white text-xl font-bold">${(financialStats?.stats?.byServiceCategory?.promo_services || 0).toFixed(0)}</p>
                  </div>
                  <div className="bg-gray-800 p-4 rounded-lg">
                    <p className="text-gray-400 text-xs mb-1">Other</p>
                    <p className="text-white text-xl font-bold">${(financialStats?.stats?.byServiceCategory?.other || 0).toFixed(0)}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
