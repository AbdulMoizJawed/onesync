...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'notifications' | 'users' | 'artists' | 'releases' | 'approvals' | 'ready' | 'analytics' | 'takedowns' | 'payouts' | 'reviews'>('overview')
  ...
  // Admin authentication check - only allow @onesync.music emails
  useEffect(() => {
    if (!authLoading) {
      if (!user) {
  ...
  useEffect(() => {
    // Only load platform data if user is authenticated and is admin
  ...
  useEffect(() => {
    // Load pending releases for approval (only for admin users)
    if (user && isAdminUser(user)) {
  ...
  const handleReleaseApproval = async (releaseId: string, approved: boolean) => {
    try {
      ...
      if (error) {
        ...
        toast.error(`Failed to ${approved ? 'approve' : 'reject'} release`)
      } else {
        toast.success(`Release ${approved ? 'approved' : 'rejected'} successfully!`)
      ...
      toast.error('Network error while updating release')
    } finally {
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-950">
        <Card className="w-96 bg-gray-900 border-gray-800">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-gray-400 mb-4">You must be logged in to access the admin dashboard.</p>
            <Button onClick={() => router.push('/auth/login?redirect=/admin')} className="bg-white text-gray-950 hover:bg-gray-100">
              Go to Login
            </Button>
          </CardContent>
        </Card>
        ...
        <Card className="w-96 bg-gray-900 border-gray-800">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-gray-400 mb-4">
              This area is restricted to OneSync administrators only.
              <br />
              <span className="text-sm text-gray-500 mt-2 block">Email: {user.email}</span>
            </p>
            <Button onClick={() => router.push('/')} className="bg-white text-gray-950 hover:bg-gray-100">
              Return to Home
            </Button>
          </CardContent>
        </Card>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {loading ? '...' : releases.filter(r => r.status === 'approved').length}
                  </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center space-x-2">
                  ...
                  <span>Ready for Delivery ({releases.filter(r => r.status === 'approved').length} releases)</span>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {releases.filter(r => r.status === 'approved').length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-500">No approved releases ready for delivery</p>
                      <p className="text-sm text-gray-400 mt-2">Releases will appear here once approved in the Release Approvals tab</p>
                    </div>
                  ) : (
                    <>
                      <div className="mb-4 text-sm text-green-600 font-medium">
                        ✅ {releases.filter(r => r.status === 'approved').length} releases approved and ready for distribution
                      </div>
                      {releases.filter(r => r.status === 'approved').map((release) => (
                        <Card key={release.id} className="p-6 border border-green-200 bg-green-50 hover:bg-green-100 transition-all">
                          <div className="flex items-start space-x-4">
                            {/* Artwork */}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between">
                                <div>
                                  ...
                                  <div className="flex items-center space-x-4 mt-2 text-sm text-gray-500">
                                    ...
                                    <span>•</span>
                                    <span>
                                      Approved {new Date(release.created_at).toLocaleDateString()}
                                    </span>
                                  </div>
                                  {release.release_date && (
                                    <p className="text-sm text-gray-500 mt-1">
                              ...
                              {/* Audio Player */}
                              {release.audio_url && (
                                <div className="flex items-center space-x-3 mt-4 p-3 bg-white rounded-lg border">
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => toggleAudio(release.id, release.audio_url)}
                                    className="flex items-center space-x-2"
                                  >
                                    {currentlyPlaying === release.id ? (
                                      <Pause className="h-4 w-4" />
                              ...
                              <div className="flex items-center space-x-2 mt-4">
                                <Button
                                  ...
                                  onClick={() => {
                                    toast.success('Release sent for distribution!')
                                    // Here you would trigger the actual distribution process
                                  }}
                                >
                                ...
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => downloadReleasePackage(release)}
                                >
                                  <Download className="h-4 w-4 mr-1" />
                                  Download Package
                                </Button>
