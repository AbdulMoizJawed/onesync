...
const ArtistSearch = dynamic(
  () =>
    import("@/components/artist-search").then((mod) => ({
      default: mod.ArtistSearch,
    })),
  {
    ssr: false,
    loading: () => <p>Loading...</p>,
...
const ArtistCard = React.memo(
  ({
    ...
    return (
      <Card
        className={`card-dark animate-dissolve-in hover-lift-gentle stagger-${Math.min(
          index + 1,
          5
        )} overflow-hidden`}
      >
        <CardHeader className="flex flex-row items-start justify-between pb-3 px-3 sm:px-6 pt-3 sm:pt-6">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="bg-gray-800 border-gray-700 text-white animate-slide-in-down"
            >
              <DropdownMenuItem
                onClick={() => onEdit(artist)}
                className="hover:bg-gray-700 hover-scale press-effect"
              >
        ...
        <CardContent className="pt-0 px-3 sm:px-6 pb-3 sm:pb-6">
          ...
          <div className="space-y-1.5 sm:space-y-2">
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <Badge
                variant="secondary"
                className="status-success text-xs px-1.5 py-0.5"
              >
                Active
              </Badge>
            </div>
    ...
    try {
      ...
      if (response.ok) {
        if (result.updated > 0) {
          setError(null);
          toast.success(
            `Successfully updated ${result.updated} artist images from Spotify!`
          );
          await fetchArtists();
        } else {
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className="button-primary w-full sm:w-auto text-sm sm:text-base"
                    >
                      <IconComponent
                        name="addCircle"
              ...
              <DialogContent className="bg-gray-900 border-gray-700 text-white w-[calc(100%-2rem)] max-w-md mx-auto max-h-[85vh] sm:max-h-[90vh] flex flex-col p-0">
                ...
                <DialogFooter className="px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-800 flex-row gap-2">
                  ...
                  <Button
                    ...
                    disabled={
                      saving || !artistName.trim() || !selectedSpotifyArtist
                    }
                  >
